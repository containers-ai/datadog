...
check_previous_build_tag_and_do_download_for_upgrade(){
    ...
    if [ "0$previous_tag_first_digit" -ge "5" ] && [ "0$tag_middle_digit" -gt "0$previous_tag_middle_digit" ]; then
        ...
        if ! curl -sL --fail https://raw.githubusercontent.com/containers-ai/datadog/${previous_fed_tag}/deploy/backup-restore.sh -O; then
...
get_repo_url()
{
    ...
    do
        read -r -p "$(tput setaf 2)Please enter the URL of private repository (e.g., repo.example.com/datadog): $(tput sgr 0) " repo_url </dev/tty
        repo_url=$(echo "$repo_url" | tr '[:upper:]' '[:lower:]')
    done

    # For install.sh
    export FEDERATORAI_FILE_PATH=$repo_url
}
